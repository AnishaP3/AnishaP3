/*
 * Asks the user to enter their credit card information so they can pay their bills.
...
public class GasPayments {
  ...
  //Method to get cost of the gas bill from the gasBill file
  public ArrayList<String[]> getCostOfGasBillFromFile() {
    ...
    //Reading the gasBill file to get the cost of the gas bill
    try {
  ...
  //Method to update the amount paid in the gas file
  public void updateTheAmountInGasFile() {
    ...
    // Reading the gasBill file and finding the matching username and month 
    // to get the lines that need to be changed 
    // and save the rest of the lines
    try {
      FileReader fr = new FileReader (GAS_BILL_FILE);
      BufferedReader br = new BufferedReader(fr);
      line = br.readLine();
      splitLine = new String[line.split(",").length];
      while(line!= null) {
        // if the username matches add all user lines to 
        ...
        //if username and month matches, then it is the current line
          ...
          matchCounter += 1;
             //If duplicate user, add user lines to before lines
             if (matchCounter > 1) {
    ...
    // Loop to find the cost of the gas bill and amount paid in the 
    // array and add the total cost and total amount paid 
    for(String[] userNameValue : userNameValues) {
      if(userNameValue[0].contains("Cost of the gas bill")) {
    ...
    //Updating the amount paid if the user pays their gas bill
    ...
    //Printing the updated gas bill into a file
    try {
        FileWriter fw = new FileWriter(GAS_BILL_FILE);
      ...
      System.out.println("The gas bill details could not be printed into a file");
    }
