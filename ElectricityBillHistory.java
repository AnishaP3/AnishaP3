...
/*
 * Reads from the file "electrictyReading.txt" and then prints the file
 * Suha Khan
*/

public class ElectricityBillHistory { 
  ...
  //Method to print the electricity bill history to the user
  public void printingElectricityBillHistory() {
    
    //Variables
    String line = "";
    String[] splitLine = null, commaSplitLine = null, colonSplitLine = null; 

    // Reading the electricity bill file to find the username of the user 
    // and printing those lines to the user
    try {
      FileReader fr = new FileReader("electricityBill.txt");
      ...
      while(line != null) {
        if (line.contains(user.getUsername())) {
          ...
          for (int i = 0; i < commaSplitLine.length; i++) {
              ...
              // If the username is equal to the username that is split from its heading,    
              // and print the line
